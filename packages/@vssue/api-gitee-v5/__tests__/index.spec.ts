...
describe('methods', () => {
  ...
  test('redirectAuth', () => {
    ...
    expect(window.location.href).toBe(
      `${baseURL}/oauth/authorize?client_id=${
        ...
        url
      )}&scope=user_info%20issues%20notes&response_type=code&state=${
        options.state
      }`
    );
  ...
  test('postIssue', async () => {
    ...
    mock
      .onPost(new RegExp(`repos/${options.owner}/issues$`))
  ...
  describe('getComments', () => {
    ...
    beforeEach(() => {
      mock
        ...
        .reply(200, fixtures.comments, {
          total_count: fixtures.comments.length,
        });
    });
    ...
    test('login', async () => {
      ...
      expect(request.headers.Accept).toEqual(
        expect.arrayContaining(['application/vnd.gitee.html+json'])
      );
      expect(request.params.access_token).toBe(mockToken);
    });
