...
describe('methods', () => {
  ...
  test('redirectAuth', () => {
    ...
    API.redirectAuth()
    expect(window.location.href).toBe(`${baseURL}/oauth/authorize?client_id=${options.clientId}&redirect_uri=${encodeURIComponent(url)}&response_type=token&state=${options.state}`)
  ...
  describe('getIssue', () => {
    describe('with issue id', () => {
      ...
      describe('issue exists', () => {
        beforeEach(() => {
          mock.onGet(new RegExp(`projects/${encodedRepo}/issues/${issueId}$`)).reply(200, fixtures.issue)
        })
      ...
      test('issue does not exist', async () => {
        mock.onGet(new RegExp(`projects/${encodedRepo}/issues/${issueId}$`)).reply(404)
        const issue = await API.getIssue({
          issueId,
          accessToken: null,
        })
      ...
      test('error', async () => {
        mock.onGet(new RegExp(`projects/${encodedRepo}/issues/${issueId}$`)).reply(500)
    ...
    describe('with issue title', () => {
      ...
      describe('issue exists', () => {
        beforeEach(() => {
          mock.onGet(new RegExp(`projects/${encodedRepo}/issues$`)).reply(200, fixtures.issues)
        })
      ...
      test('issue does not exist', async () => {
        mock.onGet(new RegExp(`projects/${encodedRepo}/issues$`)).reply(200, [])
        const issue = await API.getIssue({
          issueTitle,
          accessToken: null,
        })
  ...
  test('postIssue', async () => {
    ...
    mock.onPost(new RegExp(`projects/${encodedRepo}/issues$`)).reply(201, fixtures.issue)
  ...
  describe('getComments', () => {
    ...
    beforeEach(() => {
      mock
        ...
        .onGet(new RegExp(`projects/${encodedRepo}/issues/${issueId}/notes/\\d*/award_emoji$`))
  ...
  test('postComment', async () => {
    ...
    mock.onPost(new RegExp(`projects/${encodedRepo}/issues/${issueId}/notes$`)).reply(201, fixtures.comment)
  ...
  test('putComment', async () => {
    ...
    const content = fixtures.comment.body
    mock.onPut(new RegExp(`projects/${encodedRepo}/issues/${issueId}/notes/${commentId}$`)).reply(201, fixtures.comment)
      .onPost(new RegExp(`markdown$`)).reply(200, { html: '<p>Faked HTML body</p>' })
      .onGet(new RegExp(`projects/${encodedRepo}/issues/${issueId}/notes/\\d*/award_emoji$`)).reply(200, fixtures.reactions)
  ...
  test('deleteComment', async () => {
    const issueId = 1
    const commentId = fixtures.comment.id
    mock.onDelete(new RegExp(`projects/${encodedRepo}/issues/${issueId}/notes/${commentId}$`)).reply(204)
  ...
  test('getCommentReactions', async () => {
    const issueId = 1
    const commentId = fixtures.comment.id
    mock.onGet(new RegExp(`projects/${encodedRepo}/issues/${issueId}/notes/${commentId}/award_emoji$`)).reply(200, fixtures.reactions)
  ...
  test('postCommentReaction', async () => {
    const issueId = 1
    const commentId = fixtures.comment.id
    mock.onPost(new RegExp(`projects/${encodedRepo}/issues/${issueId}/notes/${commentId}/award_emoji$`)).reply(201)
