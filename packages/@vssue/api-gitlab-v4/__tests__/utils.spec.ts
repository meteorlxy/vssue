...
test('normalizeReactions', () => {
  ...
  expect(reactions.like).toBe(
    fixtures.reactions.filter(item => item.name === 'thumbsup').length
  );
  expect(reactions.unlike).toBe(
    fixtures.reactions.filter(item => item.name === 'thumbsdown').length
  );
  expect(reactions.heart).toBe(
    fixtures.reactions.filter(item => item.name === 'heart').length
  );
});
