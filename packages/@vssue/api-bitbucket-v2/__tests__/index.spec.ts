...
import { normalizeUser, normalizeIssue, normalizeComment } from '../src/utils';
...
describe('methods', () => {
  ...
  test('redirectAuth', () => {
    ...
    expect(window.location.href).toBe(
      `${baseURL}/site/oauth2/authorize?client_id=${
        options.clientId
  ...
  describe('getIssue', () => {
      ...
      describe('issue exists', () => {
        beforeEach(() => {
          mock
            .onGet(
              new RegExp(
                `repositories/${options.owner}/${options.repo}/issues/${issueId}$`
              )
            )
    ...
    describe('with issue title', () => {
      ...
      describe('issue exists', () => {
        beforeEach(() => {
          mock
            .onGet(
              new RegExp(
                `repositories/${options.owner}/${options.repo}/issues$`
              )
            )
      ...
      test('issue does not exist', async () => {
        mock
          .onGet(
            new RegExp(`repositories/${options.owner}/${options.repo}/issues$`)
          )
          .reply(200, []);
        const issue = await API.getIssue({
          issueTitle,
  ...
  test('postIssue', async () => {
    ...
    expect(issue).toEqual(
      normalizeIssue(
        Object.assign({}, fixtures.issue, {
          links: {
            html: {
              href: concatURL(
                baseURL,
                `${options.owner}/${options.repo}/issues/${fixtures.issue.id}`
              ),
            },
          },
        })
  ...
  describe('getComments', () => {
    ...
    beforeEach(() => {
      mock
        .onGet(
          new RegExp(
            `repositories/${options.owner}/${options.repo}/issues/${issueId}/comments$`
          )
        )
    ...
    describe('query', () => {
      ...
      test('common', async () => {
        ...
        expect(comments.data).toEqual(
          fixtures.comments.values.slice(0, query.perPage).map(normalizeComment)
        );
      });
